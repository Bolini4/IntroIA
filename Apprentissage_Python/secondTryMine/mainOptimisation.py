...
images_train, images_test, images_validation, labels_train, labels_test, labels_validation = load_data('./images/new')
...
for i in range(1, 25):
    ...
    history = model.fit(epochs=i,batch_size=1, x=images_train, y=labels_train, validation_data=(images_validation, labels_validation))
    scores = model.evaluate(images_test, labels_test)
    print('Accuracy: {}'.format(scores[1]))
    accuracyHistory1092.append(scores[1])
...
for i in range(1, 25):
    ...
    scores = model.evaluate(images_test, labels_test)
    print('Accuracy: {}'.format(scores[1]))
    accuracyHistory546.append(scores[1])

for i in range(1, 25):
    model = Sequential()

    model.add(Dense(64,input_dim = 28*28, activation='relu'))
    model.add(Dense(2184,activation='relu'))
    model.add(Dense(10, activation='softmax'))
    ...
    scores = model.evaluate(images_test, labels_test)
    print('Accuracy: {}'.format(scores[1]))
    accuracyHistory2184.append(scores[1])


# Affichage de l'historique de la précision
# plt.plot(history.history['accuracy'], label='Training Accuracy')
plt.plot(accuracyHistory1092, label=' Accuracy 1092')
plt.plot(accuracyHistory546, label=' Accuracy 546')
plt.plot(accuracyHistory2184, label=' Accuracy 2184')
plt.title('Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
